...
def get_user_keyboard(user_id):
    ...
    if user_id not in users or datetime.datetime.now() > datetime.datetime.strptime(users[user_id], '%Y-%m-%d %H:%M:%S'):
        # If no access, return redeem key keyboard
        return ReplyKeyboardMarkup([
            [KeyboardButton('/redeem')],
            [KeyboardButton('/help')]
        ], resize_keyboard=True)
    
    # If admin, return full keyboard with admin commands
    if str(user_id) in ADMIN_IDS:
        return ReplyKeyboardMarkup([
            [KeyboardButton('/genkey'), KeyboardButton('/allusers')],
            [KeyboardButton('/start'), KeyboardButton('/stop')],
            [KeyboardButton('/bgmi'), KeyboardButton('/broadcast')],
            [KeyboardButton('/help')]
        ], resize_keyboard=True)
    ...
    return ReplyKeyboardMarkup([
        [KeyboardButton('/start'), KeyboardButton('/stop')],
        [KeyboardButton('/bgmi'), KeyboardButton('/redeem')],
        [KeyboardButton('/help')]
    ], resize_keyboard=True)
...
async def genkey(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await update.message.reply_text(response, reply_markup=get_user_keyboard(user_id))
